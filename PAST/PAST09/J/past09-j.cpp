...
void paint_point( std::vector< std::vector< int > >& board, int x, int y, int r, int ud, int lr, int N ) 
{
    int h, w;
    if( r == 0 ) {
        h = x; w = y;
...
std::vector< std::vector< int > > rotate_board( const std::vector< std::vector< int > >& board, int r )
{
    int N = board.size();
...
int main()
{
    ...
    //  Modes
    //  r == 0 if 0 deg., 1 == 90 deg., 2 == 180 deg., 3 == 270 deg.
    int r = 0;
    ...
    for( int i = 0; i < Q; i ++ ) {
        ...
        if( q == 1 ) {
            int x, y;
            std::cin >> x >> y;
            paint_point( board, x - 1, y - 1, r, ud, lr, N );
        }
        else if( q == 2 ) {
            char c;
            std::cin >> c;
